var makeGraph = require("../graph/make_graph"),
	...
	addUmdShim = require("../bundle/add_umd_shim"),
...
var pluginifier = function(config, pluginifierOptions){
	...
	return makeGraph(config, pluginifierOptions).then(function(data){
		...
		var pluginify = function(moduleNames, options){
			...
			var isGlobalOrUmd = ["global", "umd"].indexOf(options.format) >= 0;
			var transpiledFormat = isGlobalOrUmd ? "amd" : options.format;
			transpile(nodesInBundle, transpiledFormat, options);
			...
			if(options.format === "umd") {
				addUmdShim(bundle, options);
			}
			concatSource(bundle,"activeSource", isGlobalOrUmd);
